...
subtitle: "with `{ospsuite}` and `{esqlabsR}`"
module: "Agenda + Day-of Deck"
ID: 02
date: last-modified
author:
  - Pavel Balazki
...
Download both files and unzip them on your machine:

:::: {.columns}
::: {.column width="50%"}
::: {.center-x}
![](../workshops/resources/A1/esqlabsR.png){height=120 fig-align="center"}

**Workshop project**

[⬇ Midazolam-Workshop.zip](https://github.com/esqLABS/PAGE2026-R-WORKSHOP/releases/download/workshop-materials/Midazolam-Workshop.zip)
:::
:::
::: {.column width="50%"}
::: {.center-x}
![](../workshops/resources/A1/esqapp.png){height=120 fig-align="center"}

**ESQapp**

[⬇ ESQApp_1.0.0_x64-setup.zip](https://github.com/esqLABS/PAGE2026-R-WORKSHOP/releases/download/workshop-materials/ESQApp_1.0.0_x64-setup.zip)
:::
:::
::::
...
3. Select the **Midazolam-Workshop** folder on the Desktop
...
::: {.r-fit-text}
**Load an OSP model in R · inspect it's structure · save it back.**
:::
...
Open `exercises/block1.R` in RStudio. Tasks:
...
::: {.notes}
Solutions in `exercises/solutions/block1.R`. Common gotchas: forgetting `system.file`, escaping `|` in regex (don't — these are not regex), confusing `*` and `**` wildcards.
:::
...
Compare default Aciclovir vs. a "no GFR" variant. Open `exercises/block2.R`.
...
::: {.center-x}
⏰ **30 min** · Solutions in `exercises/solutions/block2.R`
:::
...
::: {.r-fit-text}
**Move from "scripts in a folder" to a regulatory-grade, Snapshot-driven project.**
:::
...
::: {.center-x}
**Use case for the rest of the day:** assemble a reproducible Midazolam M&S project that survives a quality audit.
:::
...
initProject(path = "~/Workshop/Midazolam-Workshop")
...
::: {.r-fit-text}
**Switch to ESQapp.** Walk through the Midazolam-Workshop project definitions live.
:::

::: {.incremental}
- Open `Midazolam-Workshop/ProjectConfiguration.json` in ESQapp
...
Open the **Midazolam-Workshop** project in **ESQapp**. Build a new scenario from scratch:
...
::: {.incremental}
- Load + manipulate any PK-Sim/MoBi model in R
...
**One thing to add / change next year?** — scan the QR or email us!
:::

::: {.column width="40%"}
::: {.center-x}
![](../resources/feedback-qr.png){height=320 fig-align="center"}

_Scan to leave us feedback_
:::
:::
::::

::: {.notes}
INSTRUCTOR: count hands per option, then go around the room for one-word suggestions. Encourage everyone to scan the QR for written feedback. Capture flipchart photos for post-workshop notes.
:::